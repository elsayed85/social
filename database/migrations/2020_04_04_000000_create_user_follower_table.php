...
class CreateUserFollowerTable extends Migration
{
    ...
    public function up()
    {
        Schema::create(config('follow.relation_table', 'user_follower'), function (Blueprint $table) {
            ...
            $table->foreign('following_id')->on('users')->references('id')->cascadeOnDelete()->cascadeOnUpdate();
            $table->foreign('follower_id')->on('users')->references('id')->cascadeOnDelete()->cascadeOnUpdate();
            $table->timestamps();
        });
    }
